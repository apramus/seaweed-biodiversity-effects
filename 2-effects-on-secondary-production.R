...
df <- sum[,c("Week", "Block", "ProDivTrtType", "ProDivTrtRich", "ProDivTrtID", "ProBiomass", "ConAbund", "ConBiomass", "ConDivRich")]
...
melt$variable <- ifelse(melt$variable=="ConAbund", paste("Consumer abundance (#)"), paste(melt$variable))
melt$variable <- ifelse(melt$variable=="ConBiomass", paste("Consumer biomass (g)"), paste(melt$variable))
melt$variable <- ifelse(melt$variable=="ConDivRich", paste("Consumer richness (# of taxa)"), paste(melt$variable))
melt$variable <- factor(melt$variable, levels=c("Consumer abundance (#)",  "Consumer biomass (g)", "Consumer richness (# of taxa)"))
...
pAll <- ggplot(aes(x=ProDivTrtID, y=mean), data = melt.summary) +
   ...
   theme(
      ...
      axis.text.x = element_text(angle=90, hjust=1, vjust=0.4, face=c( "italic", "italic", "italic", "italic", "plain", "plain", "plain")), 
...
pAll <- pAll + 
   geom_text(aes(label=lab), data=labs, parse=T, vjust=-0.75, hjust=1.1, size=2) +
   geom_text(aes(label=panel, x=-Inf, y=Inf), data=labs, fontface="bold", vjust=1.75, hjust=-0.75, size=3.25)
   #vjust=2, hjust=1.25, parse=T, size=1.8)
   #lab(x="Macroalgal diversity treatment")
   #theme(aspect.ratio=1)
   #ylab(paste(eval(plot.name)))
print(pAll)
ggsave(pAll, file="./figures/figure-1-effects-on-secondary-production.jpg", dpi=1200, height=5, width=2.5)
...
print(histograms)
ggsave(histograms, file="./figures/histograms-of-consumer-response-distributions.jpg", dpi=1200, height=8, width=10.5)
...
write.csv(stats, "./derived-data/anova-summary-table-for-effects-on-consumer-responses.csv")

# tukeys hsd for consumer responses

# subset by respnse variable
ca <- subset(melt.summary, variable == "Consumer abundance (#)")
cb <- subset(melt.summary, variable == "Consumer biomass (g)")
cr <- subset(melt.summary, variable == "Consumer richness (# of taxa)")
...
t1$response <- "Consumer abundance"
t2 <- as.data.frame(tcb$ProDivTrtID)
t2$response <- "Consumer biomass"
t3 <- as.data.frame(tcr$ProDivTrtID)
t3$response <- "Consumer richness"
t <- as.data.frame(rbind(t1, t2, t3))

# save Tukeys as .csv for comparison
write.csv(t, "./derived-data/tukeys-summary-table-for-effects-on-consumer-responses.csv")