...
melt$variable <- ifelse(melt$variable=="ProBiomass2", paste("Producer biomass (g)"), paste(melt$variable))
...
melt$variable <- factor(melt$variable, levels=c("Consumer abundance (#)",  "Consumer biomass (g)", "Consumer richness (# of taxa)", "Producer biomass (g)"))
...
melt.summary$variable <- factor(melt.summary$variable, levels = c("Consumer abundance (#)",  "Consumer biomass (g)", "Consumer richness (# of taxa)", "Producer biomass (g)"))
...
pAll <- ggplot(aes(x=ProDivTrtID, y=mean), data = melt.summary) +
   facet_wrap(~factor(variable, levels=c("Consumer abundance (#)",  "Consumer biomass (g)", "Consumer richness (# of taxa)", "Producer biomass (g)")), scales = "free_y", ncol=1, strip.position="left") +
...
pAll <- pAll + 
   ...
   geom_text(aes(label=panel2, x=-Inf, y=Inf), data=labs, fontface="bold", vjust=1.75, hjust=-0.5, size=3)
...
pb <- subset(melt.summary, variable == "Producer biomass (g)")
