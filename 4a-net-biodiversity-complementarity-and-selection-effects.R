...
#cumStats$hjust <- 1.1
#cumStats$hjust[10] <- 1.43
#cumStats$hjust[12] <- 1.43
...
pAll <- ggplot(aes(x=trt.id, y=value), data = melt) +
   ...
   facet_grid2(response~variable, scales="free_y", switch="y", remove_labels = "all", axes="all", labeller = labeller(variable = label_wrap_gen(width=17))) + #, strip.position.y="left") +
...
pAll
#ggsave(pAll, file="./figures-and-tables/Figure_4.pdf", dpi=1200, height=6, width=3)  # previously width = 4.33
#tiff(filename="./figures-and-tables/Figure_4.tiff", height=6*800, width=3*800, units="px", res=800, compression="lzw")
#print(pAll)
#dev.off()
...
# means$response <- ifelse(means$response=="Invertebrate abundance (#)", paste("Invertebrate abundance\n(#)"), paste(means$response))
# means$response <- ifelse(means$response=="Invertebrate richness (# of taxa)", paste("Invertebrate richness\n(# of taxa)"), paste(means$response))
# means$response <- ifelse(means$response=="Invertebrate biomass (g)", paste("Invertebrate biomass\n(g)"), paste(means$response))
# means$response <- ifelse(means$response=="Macroalgal wet mass (g)", paste("Macroalgal wet mass\n(g)"), paste(means$response))
# means$response <- factor(means$response, levels=c("Invertebrate abundance\n(#)", "Invertebrate biomass\n(g)", "Invertebrate richness\n(# of taxa)", "Macroalgal wet mass\n(g)"))
...
geom_errorbar(aes(ymin = ifelse(mean>=0, mean, mean-se), ymax = ifelse(mean>=0, mean+se, mean), fill=variable), color="black", width=0.15, position=position_dodge(0.7), size=0.25) +
   #geom_errorbar(data=means2, aes(ymin = mean -se, ymax = mean, fill=variable), color="black", width=0.15, position=position_dodge(0.7), size=0.5) +
   ...
   #scale_fill_manual(values=c("gold", "firebrick", "orange"), labels = c("Complementarity effect", "Selection effect", "Net biodiversity effect")) +
   #scale_color_manual(values=c("gold", "firebrick", "orange"), labels = c("Complementarity effect", "Selection effect", "Net biodiversity effect")) +
  # geom_text(aes(label=let), x=-Inf, y=Inf, fontface="bold", vjust=1.25, hjust=-1, size=2.75) +
   #geom_text(data=fslabs, aes(label=lab), x=Inf, y=Inf, parse=T, vjust=1.25, hjust=1, size=2.25) +
   #labs(y="Foundation species\n(quadrat ± SE, n = 12)") +
   ...
   theme(
      ...
      #axis.text.x = element_text(angle=90, hjust=1, vjust=1, face=c( "italic", "italic", "italic", "italic", "plain", "plain", "plain")), 
      ...
      #legend.text = element_text(hjust=0, size=rel(0.8)), 
...
ggsave(pmeans, file="./figures-and-tables/Figure_4.pdf", dpi=1200, height=4.5, width=3)  # previously width = 4.33

tiff(filename="./figures-and-tables/Figure_4.tif", height=4.5*800, width=3*800, units="px", res=800, compression="lzw")
print(pmeans)
dev.off()
...
#pmeans <- pmeans + 
   ...
   #ylab(paste(eval(plot.name)))
#pmeans
#ggsave(pmeans, file="./figures-and-tables/Figure4Redux.pdf", dpi=1200, height=6, width=3)  # previously width = 4.33
