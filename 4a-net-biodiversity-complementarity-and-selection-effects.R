...
df0[,c("ConAbund", "ConBiomass", "ConDivRich")] <- as.numeric(apply(df0[,c("ConAbund", "ConBiomass", "ConDivRich")], 2, function(x) ifelse(df0$ProBiomass>0, x, NA)))
df0
...
colnames(sdf0)[5:7] <- c("ConAbund", "ConBiomass", "ConDivRich")
...
write.csv(cumStats, "./derived-data/anova-summary-table-for-effects-on-partition.csv")
str(cumTPP)
...
pAll <- pAll + 
   geom_text(aes(label=lab, vjust=vjust), data=cumStats, parse=T,  hjust=1.1, size=2) +
   geom_text(aes(label=panel, x=-Inf, y=Inf), data=cumStats, fontface="bold", vjust=1.75, hjust=-0.75, size=3.25)
   #vjust=2, hjust=1.25, parse=T, size=1.8)
   #lab(x="Macroalgal diversity treatment")
   #theme(aspect.ratio=1)
   #ylab(paste(eval(plot.name)))
pAll
ggsave(pAll, file="./figures/figure-3-net-biodiversity-complementarity-and-selection-effects.jpg", dpi=1200, height=5, width=5)
