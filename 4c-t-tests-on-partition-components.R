...
cumTPP <- read.csv("./derived-data/partitioned-effects-on-response-metrics.csv")
cumTPP$trt <- factor(cumTPP$trt, levels = c("NM", "IM", "CM"))
cumTPP$trt.id <- gsub("invasive", "nonnative", cumTPP$trt.id)
cumTPP$trt.id <- factor(cumTPP$trt.id, levels = c("3 species native", "3 species nonnative", "4 species"))
...
# this is performed on the transformed biodiversity responses of each response variable
...
labs <- data.frame(trt.id=levels(melt.ca.ce$trt.id), 
        t=round(sapply(fits, function(x) x$'statistic'[[1]]), 3),
...
colnames(t.tests) <- c("Response", "Component", "Treatment", "t-Value", "DF", "P-Value")

t.tests$sig <- ifelse(t.tests$"P-Value"<0.05, paste("*"), paste(""))
t.tests$sig <- ifelse(t.tests$"P-Value"<0.01, paste("**"), t.tests$sig)
t.tests$sig <- ifelse(t.tests$"P-Value"<0.001, paste("***"), t.tests$sig)
t.tests$"P-Value" <- ifelse(t.tests$"P-Value"<0.001, paste("<0.001"), t.tests$"P-Value")
t.tests

write.csv(t.tests, "./results-summary-tables/Table-S3.csv")
